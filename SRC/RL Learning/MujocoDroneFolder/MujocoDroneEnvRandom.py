...
class MjDroneEnvRand(gym.Env):
    metadata = {'render.modes': ['human','None'], "render_fps":4}
    def __init__(self, render_mode):
        super(MjDroneEnvRand, self) .__init__()
        # Load Mujoco Model
        ...
        # This is where the drone is at start
        NewXY = random.sample(range(-5,5),2)
        NewZ = 0
        NewPos = np.hstack((NewXY, NewZ))
        NpNewPos = np.array(NewPos)
        self.data.qpos[0:3] = NpNewPos
        
        # This is where the drone wants to go
        NewTargetPosXY = random.sample(range(-5,5),2)
        NewTargetPosZ = random.sample(range(2,4),1)
        NewTargetPos = np.hstack((NewTargetPosXY, NewTargetPosZ))
        NpNewTargetPos = np.array(NewTargetPos)
        self.targetLocation = np.array([0,0,2,1,0,0,0])
        self.targetLocation[0:3] = NpNewTargetPos
    ...
    def get_obs(self):
        # return distances to target location, return current velocity
        return np.concatenate((self.data.qpos - self.targetLocation, self.data.qvel), dtype=np.float32)
    
    def trunc_func(self):
        # Simulation time cutoff = 5 seconds
        if self.data.time > 5:
            return True
        else:
            return False
    ...
    def render(self):
        # Render mode human uses regular passive viewer
        # Render mode None uses no rendering
